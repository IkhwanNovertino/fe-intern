...
export default function VacancyDescription(props) {
  ...
  useEffect(() => {
    if (id === '1') {
      setVacant({
        id: '1',
        position: 'buat pengajuan mandiri',
        job_desc: '',
        requirement: [],
        duration: 'xxxx',
        start_an_intern: new Date(),
        createdAt: new Date(),
      })
    } else {
      axios.get(`${ROOT_API}/${API_VERSION}/vacancy/${id}`)
        .then(res => {
        ...
        <div className="w-full grid gap-2 sm:grid-rows-2 sm:grid-flow-col">
          <div>
            <p className="flex text-xs font-normal text-light">
              ...
              <span className="ps-1">
                Mulai magang {vacant.id === '1' ? 'xxxx' : format(vacant.start_an_intern, 'dd MMMM yyyy')}
              </span>
            </p>
          </div>
          <div>
          ...
          <div>
            <p className="flex text-xs font-normal text-light">
              ...
              <span className="ps-1">
                Dibuat {vacant.id === '1' ? 'xxxx' : format(vacant.createdAt, 'dd MMMM yyyy')}
              </span>
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <section>
            <ul className="ps-4 list-disc list-outside text-light">
              {vacant.requirement.map((item, index) => (
